...
use crate::torrent::{File, Key, Torrent};
...
pub(crate) async fn all(torrent: &Torrent) -> anyhow::Result<Downloaded> {
    println!("{:?}", torrent);
    let tracker_resp = TrackerResponse::query(torrent)
        .await
    ...
    for piece_i in 0..torrent.info.pieces.0.len() {
        let piece = Piece::new(piece_i, torrent, &peers);
    ...
    let mut downloaded_pieces = vec![0; torrent.length()];
    while let Some(piece) = pieces_to_download.pop() {
        ...
        downloaded_pieces[piece.index() * torrent.info.piece_length..][..piece_size]
            .copy_from_slice(&downloaded_blocks)
    }

    let files = match &torrent.info.key {
