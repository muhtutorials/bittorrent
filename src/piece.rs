...
impl Piece {
    pub(crate) fn new(index: usize, torrent: &Torrent, peers: &[Peer]) -> Self {
        let length = if index == torrent.info.pieces.0.len() - 1 {
            // calculates last piece's size
            torrent.length() % torrent.info.piece_length
        } else {
            torrent.info.piece_length
        };
        let hash = torrent.info.pieces.0[index];
        let peers = peers
            .iter()
            .enumerate()
