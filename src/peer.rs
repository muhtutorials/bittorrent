...
// TODO: should keep track of what pieces we downloaded and references to them
        ...
        'job: loop {
            while self.chocked {
                ...
                match msg.typ {
                    ...
                    | MessageType::Request | MessageType::Cancel => {
            ...
            loop {
                ...
                match msg.typ {
                    ...
                    MessageType::Piece => {
                        let piece_response =
                            PieceResponse::ref_from_bytes(&msg.payload[..])
...
impl Bitfield {
    ...
    pub(crate) fn has_piece(&self, piece_i: usize) -> bool {
        ...
        let bit_i = piece_i % 8;
        let Some(byte) = self.0.get(byte_i) else {
            return false;
        };
    ...
    pub(crate) fn pieces(&self) -> impl Iterator<Item = usize> {
        // iterates bytes
        self.0.iter().enumerate().flat_map(|(byte_i, byte)| {
            // iterates bits
...
#[test]
fn bitfield_has() {
    let bf = Bitfield(vec![0b10101010, 0b01110110]);
